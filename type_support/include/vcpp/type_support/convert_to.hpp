...
#ifndef VCPP_TYPE_SUPPORT_CONVERT_HPP
#define VCPP_TYPE_SUPPORT_CONVERT_HPP

#include "vcpp/type_support/detail/convert_to.hpp"
 ...
 *
 */
template<typename To, typename From, VCPP_REQUIRE((!std::is_same<To, From>::value))>
...
template<typename To, std::size_t n, typename From, VCPP_REQUIRE((!std::is_same<To, From>::value))>
To convert_to(const From& from){
  static_assert(detail::is_cv_type_v<To> || detail::is_cv_type_v<From>, "You cannot convert non-cv type to non-cv type! use vtype_convert instead");
...
#endif //VCPP_TYPE_SUPPORT_CONVERT_HPP
