...
#  */
#
# ifndef VCCC_HIGH_VER_FUNCTIONAL_DETAIL_INVOKE_HPP
# define VCCC_HIGH_VER_FUNCTIONAL_DETAIL_INVOKE_HPP
#
# include <utility>
# include <type_traits>
...
constexpr decltype(auto) INVOKEP_MF(invoke_category_base_of,
                                    Type T::* f, T1&& t1, Args&&... args) {
  return (std::forward<T1>(t1).*f)(std::forward<Args>(args)...);
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKEP_MF(invoke_category_reference_wrapper,
                                    Type T::* f, T1&& t1, Args&&... args) {
  return (t1.get().*f)(std::forward<Args>(args)...);
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKEP_MF(invoke_category_normal,
                                    Type T::* f, T1&& t1, Args&&... args) {
  return ((*std::forward<T1>(t1)).*f)(std::forward<Args>(args)...);
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKEP(invoke_category_base_of,
                                 Type T::* f, T1&& t1, Args&&... args) {
  return std::forward<T1>(t1).*f;
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKEP(invoke_category_reference_wrapper,
                                 Type T::* f, T1&& t1, Args&&... args) {
  return t1.get().*f;
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKEP(invoke_category_normal,
                                 Type T::* f, T1&& t1, Args&&... args) {
  return (*std::forward<T1>(t1)).*f;
}
...
template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKE_TAG1(std::true_type, Type T::* f, T1&& t1, Args&&... args)
{
  return INVOKEP_MF(get_invoke_category_t<T, T1>{}, f, std::forward<T1>(t1), std::forward<Args>(args)...);
}

template <class T, class Type, class T1, class... Args>
constexpr decltype(auto) INVOKE_TAG1(std::false_type, Type T::* f, T1&& t1, Args&&... args)
{
...
#endif //VCCC_HIGH_VER_FUNCTIONAL_DETAIL_INVOKE_HPP
