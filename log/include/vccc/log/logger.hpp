...
// TODO: add PRI.. format
template<typename CharT> bool Logger::isFormatted(const CharT& str) {
  static const std::regex fmt_reg(R"(%(?:\d+\$)?[+-]?(?:[ 0]|'.{1})?-?\d*(?:\.\d+)?[bcdeEufFgGosxXp])");
  return std::regex_search(str, fmt_reg);
}

template<typename T>
void Logger::add(const T& n) {
  out << n;
}

template<typename Arg, typename ...Args>
void Logger::add(const Arg& arg, const Args&... args) {
  add(arg);
...
template<typename Arg, typename ...Args>
void Logger::addImpl(c_printable, const Arg& arg, const Args&... args){
  if (isFormatted(arg))
...
template<typename Arg, typename ...Args>
void Logger::addImpl(not_c_printable, const Arg& arg, const Args&... args){
  return add(arg, args...);
}

template<typename ...Args>
void Logger::addFormatted(const char* fmt, const Args&... val) {
