...
#ifndef VCCC_EXPECTED_MONADIC_HPP_
#define VCCC_EXPECTED_MONADIC_HPP_
 ...
 public:
  ...
  constexpr auto operator()(F&& f, Self&& self) const {
    if (self.has_value()) {
      ...
      return vccc::invoke(std::forward<F>(f), std::forward<Self>(self).error());
    }
  }
};
 ...
 public:
  ...
  constexpr auto operator()(F&& f, Self&& self) const {
    if (self.has_value()) {
      ...
      return Expected<T, G<F, Self>>(unexpect, vccc::invoke(std::forward<F>(f), std::forward<Self>(self).error()));
    }
  }
};
...
#endif // VCCC_EXPECTED_MONADIC_HPP_
