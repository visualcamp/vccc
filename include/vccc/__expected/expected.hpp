...
#ifndef VCCC_EXPECTED_EXPECTED_HPP_
#define VCCC_EXPECTED_EXPECTED_HPP_
...
#include "vccc/__expected/bad_expected_access.hpp"
#include "vccc/__expected/monadic.hpp"
#include "vccc/__expected/unexpected.hpp"
#include "vccc/__expected/unexpect.hpp"
...
void reinit_expected_impl(std::false_type, std::false_type,
  ...
  vccc::destroy_at(vccc::addressof(old_val));
  try {
    vccc::construct_at(vccc::addressof(new_val), std::forward<Args>(args)...);
  } catch (...) {
    vccc::construct_at(vccc::addressof(old_val), std::move(temp));
    throw;
  }
}
...
constexpr void reinit_expected_impl(std::false_type, std::true_type,
  ...
  vccc::destroy_at(vccc::addressof(old_val));
  vccc::construct_at(vccc::addressof(new_val), std::move(temp));
}
...
struct expected_storage_t<false, T, E> {
  ...
  VCCC_CONSTEXPR_AFTER_CXX20 ~expected_storage_t() noexcept {}
...
struct expected_base {
  ...
  VCCC_CONSTEXPR_AFTER_CXX20 void reinit_value_and_destroy_error(Args&&... args) {
  ...
  template<typename U = T, typename Other, std::enable_if_t<!std::is_same<U, void_placeholder_t>::value, int> = 0>
  VCCC_CONSTEXPR_AFTER_CXX20 void reinit_value_and_destroy_error_from(Other&& other) {
  ...
  template<typename U = T, typename... Args, std::enable_if_t<!std::is_same<U, void_placeholder_t>::value, int> = 0>
  VCCC_CONSTEXPR_AFTER_CXX20 void reinit_error_and_destroy_value(Args&&... args) {
  ...
  constexpr void construct_value(Args&&... args) {
    vccc::construct_at(vccc::addressof(storage.value_), std::forward<Args>(args)...);
  }
  ...
  constexpr void construct_error(G&& e) {
    vccc::construct_at(vccc::addressof(storage.error_), std::forward<G>(e));
  }
  ...
  VCCC_CONSTEXPR_AFTER_CXX20 void destroy() {
  ...
  template<typename U = T, std::enable_if_t<!std::is_trivially_destructible<U>::value, int> = 0>
  VCCC_CONSTEXPR_AFTER_CXX20 void destroy_value() {
    value().~T();
  }
  ...
  template<typename U = E, std::enable_if_t<!std::is_trivially_destructible<U>::value, int> = 0>
  VCCC_CONSTEXPR_AFTER_CXX20 void destroy_error() {
    error().~E();
  }

  VCCC_NODISCARD constexpr bool has_value() const noexcept {
...
struct expected_nontrivial_dtor : expected_base<T, E> {
  ...
  VCCC_CONSTEXPR_AFTER_CXX20 ~expected_nontrivial_dtor() {
    base::destroy();
  }
...
class expected : private detail::expected_control_smf<detail::void_placdholder_or_t<T>, E> {
 ...
 public:
  ...
  constexpr const T* operator->() const noexcept {
    return vccc::addressof(base::value());
  }
 ...
 private:
  ...
  void swap_value_with_error(expected& thiz, expected& other, std::true_type /* void */, Any) noexcept(error_nothrow_swappable::value) {
    vccc::construct_at(std::addressof(thiz.error()), std::move(other.error()));
  ...
  void swap_value_with_error(expected& thiz, expected& other, std::false_type /* void */, std::true_type /* nothrow */) noexcept(nothrow_swappable::value) {
    ...
    vccc::destroy_at(std::addressof(other.error()));
    try {
      vccc::construct_at(std::addressof(other.value()), std::move(thiz.value()));
      vccc::destroy_at(std::addressof(thiz.value()));
      vccc::construct_at(std::addressof(thiz.error()), std::move(temp));
    } catch (...) {
      vccc::construct_at(std::addressof(other.error()), std::move(temp));
      throw;
    }
  }
  ...
  void swap_value_with_error(expected& thiz, expected& other, std::false_type /* void */, std::false_type /* nothrow */) noexcept(nothrow_swappable::value) {
    ...
    try {
      vccc::construct_at(std::addressof(thiz.error()), std::move(other.error()));
      vccc::destroy_at(std::addressof(other.error()));
      vccc::construct_at(std::addressof(other.value()), std::move(temp));
    } catch (...) {
      vccc::construct_at(std::addressof(thiz.value()), std::move(temp));
      throw;
    }
  }
  ...
  constexpr void check_throw() & {
    if (!has_value())
      throw bad_expected_access<std::decay_t<E>>(vccc::as_const(error()));
  }
  constexpr void check_throw() && {
    if (!has_value())
...
#endif // VCCC_EXPECTED_EXPECTED_HPP_
